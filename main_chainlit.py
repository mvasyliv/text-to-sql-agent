...
APP_PATH = APP_ROOT / "src" / "text_to_sql_agent" / "ui" / "chainlit_app.py"
...
def _build_runtime_env() -> dict[str, str]:
    """Build subprocess environment with project paths and JWT secret."""
    env = os.environ.copy()
    ...
    # Pass JWT secret via environment variable (Chainlit reads from env)
    auth_secret = os.getenv("CHAINLIT_AUTH_SECRET")
    if auth_secret:
        env["CHAINLIT_AUTH_SECRET"] = auth_secret
    else:
        print("Warning: CHAINLIT_AUTH_SECRET not set. User auth unavailable.")
...
def main() -> None:
    ...
    try:
        subprocess.run(
            command, check=True, cwd=str(APP_ROOT), env=_build_runtime_env()
        )
